...
export const nestedDataSet = (count) => {
  ...
  for (let index = 0; index < count; index++) {
    currentLevel.push({ id: index.toString(), name: `child_${index}`, children: [] });
...
export const largeDataSetShallow = (nodeCount, maxDepth) => {
  ...
  for (let index = 0; index < nodeCount; index++) {
    outerPosition.push({ id: `${index + 1}`, name: `Child_${index + 1}`, children: [] });
    let innerPosition = outerPosition[index].children;
    for (let childIndex = 0; childIndex < maxDepth; childIndex++) {
      innerPosition.push({ id: `${index + 1}_${childIndex + 1}`, name: `Child_${index + 1}_${childIndex + 1}` });
    }
  }
